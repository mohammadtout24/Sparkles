...
def add_to_cart(request, pk):
    ...
    # Add the product ID to the cart (increment quantity if it exists)
    str_pk = str(pk)
    if str_pk in cart:
        cart[str_pk] += 1
    else:
...
def cart_view(request):
    ...
    # Fetch products from database based on IDs in the cart
    products = Product.objects.filter(pk__in=cart.keys())
    ...
    for product in products:
        ...
        # Add to our list for the HTML template
        items.append({
            'product': product,
            'quantity': quantity,
            'subtotal': subtotal
        ...
        # Build the text string for the WhatsApp message
        whatsapp_message += f"- {quantity}x {product.name} (${subtotal})\n"

    whatsapp_message += f"\nTotal: ${total_price}\n\nPlease confirm availability."
    
    # Encode the message for a URL (replaces spaces with %20, etc.)
...
from .forms import ReviewForm # Make sure to import the new form
